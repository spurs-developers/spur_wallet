...
const APP = process.env.SPURS_ADMIN_APP ?? "wallet";
const TTL_MS = 60_000;

type Cache = { at: number; config: Record<string, string> };
const g = globalThis as unknown as { _walletAdminCfg?: Cache };

export async function getAdminConfig(): Promise<Record<string, string>> {
  const now = Date.now();
  if (g._walletAdminCfg && now - g._walletAdminCfg.at < TTL_MS) return g._walletAdminCfg.config;
  ...
  g._walletAdminCfg = { at: now, config };
  return config;
}
